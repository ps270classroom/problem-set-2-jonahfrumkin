...
party_incumbent_promote <- fb_ads |>
  ...
  summarise(promote_prep = mean(ad_tone_promote, na.rm = TRUE))
party_incumbent_promote

kable(party_incumbent_promote, digits = 3, col.names = c("Party", "Incumbency", "Average Promotion Ads"))
```

Democratic incumbents spend the most time trying to promote ads on average.
...
```
The shape of this histogram is right-skewed and it shows that most of the US House candidates did not make more than 10 million Facebook impressions.
\newpage
...
fb_top_plot <- ggplot(data = fb_ads |>
  ...
  geom_bar(stat = "identity", aes(fill = party)) +
    labs(title = "Top 15 US House Candidates by Facebook Impressions, 2017 - 2018",
       x = "Facebook Ad Impressions (In millions)", y = "Candidate Last Name") +
